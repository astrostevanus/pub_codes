...
from ultramodule import print_item, print_structure
specdir="./"
...
def sysrem_iter(order_spec,error,airmass):
    ...
    a_fake_std=[]
    c_fake_std=[]
    for iter in range (0,400):#,ascii="True",desc="Iteran"):
        c_i=[]
        a_j=[]
...
#rainbow-wasp33kawa_tioall--atre.h5
# rainbow-wasp33kawa_tio-9-rotfit-atre.h5
n_sys=10
whichdata="kawa"
mode="order"
mjd,bjd,rvcor,hjd,airmass=np.loadtxt("wasp33-15.info",unpack=True,dtype="float")
h5f_sysrem = h5py.File(specdir+"/post_sysrem/rainbow"+str(whichdata)+"_postsysrem-usual"+str(mode)+"_"+str(opt)+".h5", 'w')
...
print "##########################################################################"
print "SYSREM-Order/Fulbre SIM version  v1.0 (2017 August 3)"
print "\n"
...
print "##########################################################################"
print "\n"
print "Input file: rainbow-wasp33"+str(whichdata)+"_"+str(opt)+".h5"
print "# Systematics to be removed: "+str(n_sys)
print "Mode: "+str(mode)
print "Iteran: 40x"
print "Variable: "+str(vari)
print "Min. Vari.: "+str(varmin)
print "Max. Vari.: "+str(varmax)
print "\n"
for ccd in ccd_list_2breduced:
    ...
    print_structure(specdir+"rainbow-wasp33"+str(whichdata)+"_"+str(opt)+".h5")
...
#                h5f_sysrem.create_dataset(str(ccd)+"-a_std_sys"+str(vari)+"-"+str(va)+"-order-"+str(order),data=a_std_sys)
#                h5f_sysrem.create_dataset(str(ccd)+"-c_std_sys"+str(vari)+"-"+str(va)+"-order-"+str(order),data=c_std_sys)
            h5f_sysrem.create_dataset(str(ccd)+"-flux"+str(vari)+"-"+str(va)+"-order-"+str(order)+"sysrem", data=sys_flux_order)
#            h5f_sysrem.create_dataset(str(ccd)+"-sys_fr"+str(vari)+"-"+str(va)+"-order-"+str(order)+"sysrem", data= sys_sys_order)
#            h5f_sysrem.create_dataset(str(ccd)+"-a_sys"+str(vari)+"-"+str(va)+"-order-"+str(order)+"sysrem", data=a_sys)   
#            h5f_sysrem.create_dataset(str(ccd)+"-c_sys"+str(vari)+"-"+str(va)+"-order-"+str(order)+"sysrem", data=c_sys)
        if mode=="fulbre":
            h5f_sysrem.create_dataset(str(ccd)+"-a_std_sys"+str(vari)+"-"+str(va),data=a_std_sys)
            h5f_sysrem.create_dataset(str(ccd)+"-c_std_sys"+str(vari)+"-"+str(va),data=c_std_sys)
        else:
